...
const buildSpiralPaths = ({
  ...
  const [xAngleModifier, yAngleModifier] =
    direction === "cw" ? [Math.sin, Math.cos] : [Math.cos, Math.sin];
  ...
  const spiralPaths = Array.from({ length: numSpirals }).map((_, i) => {
    ...
    return Array.from({ length: NUM_POINTS_IN_SPIRAL }).reduce<string>(
      (path, _, j) => {
        ...
        const x = centerX + radius * xAngleModifier(offsetAngle);
        const y = centerY + radius * yAngleModifier(offsetAngle);
