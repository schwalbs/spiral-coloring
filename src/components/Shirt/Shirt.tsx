...
const Shirt: FC<Props> = ({}) => {
  const { colorStyle } = useContext(SelectedColorContext);
  const { numSpirals, spirals } = useContext(SpiralsContext);
      ...
      >
        ...
        {numSpirals === 1 && (
          <rect
            ...
            fill={spirals[0] ? getSpiralFill(0) : "#fff"}
          />
        )}
          ...
          <g clipPath="url(#shirt-clip-path)">
            ...
            {spiralFillPaths.map((path, i) => (
              <path
                key={i}
                d={path}
                style={{ color: spirals[i]?.hexCode ?? "transparent" }}
                fill={getSpiralFill(i)}
                stroke={colorStyle === "liquid" ? "currentColor" : "none"}
                strokeWidth="1"
              />
            ))}
